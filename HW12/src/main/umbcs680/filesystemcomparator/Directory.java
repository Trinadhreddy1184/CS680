...
public class Directory extends FSElement{
    ...
    private  LinkedList<umbcs680.filesystemcomparator.File> files;
    private  LinkedList<umbcs680.filesystemcomparator.Link> links;
    public Directory(Directory parent,String name, int size,LocalDateTime creationTime){
        super(parent,name,size,creationTime);
        this.permission="drwx-r-xr-x";
        children = new LinkedList<FSElement>();
        files = new LinkedList<umbcs680.filesystemcomparator.File>();
        subDirectories = new LinkedList<Directory>();
        links = new LinkedList<umbcs680.filesystemcomparator.Link>();
    }
    ...
    public void appendChild(FSElement child){
        ...
        }else if (child instanceof umbcs680.filesystemcomparator.Link ) {
            links.add((umbcs680.filesystemcomparator.Link) child);
        }
        else{
            files.add((umbcs680.filesystemcomparator.File) child);
        }    
        children.add(child);
    ...
    public void removeChild(FSElement child){
        if (child.isDirectory()) {
            ...
            links.remove((umbcs680.filesystemcomparator.Link) child);
        } else {
            files.remove((umbcs680.filesystemcomparator.File) child);
        }
    ...
    public  LinkedList<umbcs680.filesystemcomparator.File> getFiles(){
        return this.files;
    }
    ...
    public int getTotalSize(){
        ...
        for (umbcs680.filesystemcomparator.File f : getFiles()){
            size += f.getSize();
        }
        return size;
    }
    ...
    public LinkedList<umbcs680.filesystemcomparator.Link> getLinks() {
        return this.links;
    }
    
    // comparator getter methods

    public LinkedList<umbcs680.filesystemcomparator.Link> getLinks(Comparator<FSElement> c) {
        
        Collections.sort(this.links, c);
        return this.links;
    }

    public LinkedList<umbcs680.filesystemcomparator.File> getFiles(Comparator<FSElement> c) {
