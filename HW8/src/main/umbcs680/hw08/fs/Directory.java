...
public class Directory extends FSElement{
    ...
    private  LinkedList<umbcs680.hw08.fs.Link> links;
    public Directory(Directory parent,String name, int size,LocalDateTime creationTime){
        ...
        links = new LinkedList<umbcs680.hw08.fs.Link>();
    }
    ...
    public void appendChild(FSElement child){
        ...
        }else if (child instanceof umbcs680.hw08.fs.Link ) {
            links.add((umbcs680.hw08.fs.Link) child);
        }
        else{
            files.add((umbcs680.hw08.fs.File) child);
        }    
        children.add(child);
    }
    ...
    public void removeChild(FSElement child){
        if (child.isDirectory()) {
            ...
            links.remove((umbcs680.hw08.fs.Link) child);
        } else {
            files.remove((umbcs680.hw08.fs.File) child);
        }
    ...
    public int getTotalSize(){
        ...
        for (umbcs680.hw08.fs.File f : getFiles()){
            size += f.getSize();
        }
        return size;
    }
    ...
    public LinkedList<umbcs680.hw08.fs.Link> getLinks() {
        return this.links;
    }
}